...
def test_root_environment_example_matches_cross_layer_contract():
    expected = (
        ...
        "# Public OSM/Overpass endpoint for mapped environment geometry. OSM-derived\n"
        "# features require OpenStreetMap attribution in any user-facing surface.\n"
        "OVERPASS_API_URL=https://overpass-api.de/api/interpreter\n"
        "# OpenTopoData ASTER30m by default; this is terrain elevation, not a flood\n"
        "# forecast. Configure a compatible approved "
        "DEM/Copernicus-derived service when\n"
        "# the deployment has one.\n"
        "ELEVATION_API_URL=https://api.opentopodata.org/v1/aster30m\n"
        "# Environment observations are held only in a bounded process-local "
        "coarse cache\n"
        "# and may be served with an explicit stale warning after an upstream "
        "failure.\n"
        "# Backend-only secret required to generate real or simulation route "
        "alternatives\n"
        "# when a verified/selected destination is requested. Never pass this "
        "token to\n"
        "# Flutter; an unset or invalid value leaves routing unavailable.\n"
